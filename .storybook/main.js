module.exports = {
  ...
  webpackFinal: async (config) => {
    if (config.mode === "development") {
      ...
      config.plugins = config.plugins.filter(plugin => plugin.name !== "React Docgen Typescript Plugin")
      // Disable ESlint plugin in dev as this is also slow (and buggy due to wrong caching) => 60% speed improvement
      config.plugins = config.plugins.filter(plugin => !("options" in plugin && "eslintPath" in plugin.options));

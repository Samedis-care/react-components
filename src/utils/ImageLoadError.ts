export const isImageLoadError = (e: Error): e is ImageLoadError => {
...
export default class ImageLoadError extends Error {
	...
	constructor(mimeType: string | null) {
		super(
			`Failed to load image${mimeType ? ` of type ${mimeType}` : ""}: the format is unsupported by this browser or the file is corrupt`,
		);

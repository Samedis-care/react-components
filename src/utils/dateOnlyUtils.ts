...
export const parseDateOnly = (value: string): Date =>
	normalizeDateUtc(new Date(value));
...
const isDateOnlyAnchor = (date: Date): boolean =>
	date.getUTCHours() === 12 &&
...
export const toDateOnly = (value: Date | string | number): Date => {
	if (typeof value === "string") {
		...
		if (DATE_ONLY_ISO.test(trimmed)) return normalizeDateUtc(new Date(trimmed));
 ...
 */
export const dateOnlyFromDateTime = (value: Date | string): Date =>
	normalizeDate(value instanceof Date ? value : new Date(value));

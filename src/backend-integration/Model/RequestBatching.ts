...
class RequestBatching {
	 ...
	 */
	public static BATCH_DURATION = 10;
	/**
	 ...
	 */
	public static MAX_BATCH_TIME = 50;
	/**
	...
	>(
		...
		if (
			batchKey in this.batchPromises &&
			...
			this.batchPromises[batchKey] = promise = (async () => {
				...
				while (batchRequests.length < this.MAX_BATCH_REQUESTS) {
					...
					const nextRequestTimeWait =
						this.batchLastAdded[batchKey] + this.BATCH_DURATION - now;
					const minWait = Math.min(maxBatchTimeWait, nextRequestTimeWait);
					if (minWait <= 0) break; // something timed out, we're ready
					await sleep(minWait);
				}

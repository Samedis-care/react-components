...
const BackendMultipleSelect = (inProps) => {
    ...
    const handleLoad = useCallback(async (search, switchValue) => {
        ...
        return {
            options: await Promise.all(records.map((data) => Promise.resolve(modelToSelectorData(data)))),
    ...
    return (_jsx(BaseSelector, { multiple: true, ...otherProps, onLoad: debouncedLoad, onSelect: handleSelect, selected: selected, displaySwitch: !!switchFilterName, lru: lruConfig }));
};

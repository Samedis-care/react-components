...
const DefaultFormPageButtons = (inProps) => {
    ...
    const { showBackButtonOnly, readOnly, readOnlyReasons, dirty, isSubmitting, safeSubmit, customProps, confirmDialogMessage, autoBack, extraButtons, } = props;
    ...
    const submitAndAutoBack = useCallback(async () => {
        if (!(await safeSubmit()))
            return; // error is shown and reported regardless
        if (autoBack)
    ...
    const saveBtn = (_jsx(ActionButton, { disabled: !dirty || isSubmitting || readOnly, onClick: displayConfirmDialog ? submitWithConfirmDialog : submitAndAutoBack, children: t("common.buttons.save") }));

...
import { MatrixCellTilePropsContext, } from "./MatrixCellTileContext";
...
export const MatrixTileHighlight = styled("div", {
    ...
    boxShadow: `0 0 0 3px ${theme.palette.warning.main}, 0 0 6px 1px ${alpha(theme.palette.warning.main, 0.5)}`,
}));
...
const MatrixCellTile = (inProps) => {
    const props = useThemeProps({ props: inProps, name: "CcMatrixCellTile" });
    const { items, pairLayout = "split", splitDirection = "row", placeholder, onItemClick, renderItem, className, classes, } = props;
    ...
    if (items.length === 0) {
        ...
        content =
            placeholder != null ? (_jsx(MatrixTilePlaceholder, { className: classes?.placeholder, children: placeholder })) : null;
    }
    else if (items.length === 1 || pairLayout === "split") {
        content = items
            .slice(0, 2)
            .map((item) => (_jsx(React.Fragment, { children: wrap(item, _jsx(MatrixTileItem, { item: item })) }, item.key)));
    }
    else {
        const pair = _jsx(MatrixTileDiagonalPair, { a: items[0], b: items[1] });
        content =
            items[0].highlighted || items[1].highlighted ? (_jsx(MatrixTileHighlight, { className: classes?.highlight, children: pair })) : (pair);
    }
    ...
    const context = useMemo(() => ({ onItemClick, classes }), [onItemClick, classes]);
    return (_jsx(MatrixCellTilePropsContext.Provider, { value: context, children: _jsx(MatrixCellTileRoot, { className: combineClassNames([
                className,
                classes?.root,

...
import { MATRIX_CELL_ADD_HINT, MATRIX_CELL_SELECTED, useMatrixInteraction, } from "./MatrixInteractionContext";
export const MatrixBodyCellRoot = styled("div", {
    ...
    "&:hover [data-cc-matrix-add-chip], &:focus-within [data-cc-matrix-add-chip]": { opacity: 1, pointerEvents: "auto" },
    ...
    [`&.${matrixClasses.cellSelected}`]: {
        backgroundColor: alpha(theme.palette.info.main, theme.palette.mode === "dark" ? 0.35 : 0.25),
    },
}));
/**
 ...
 */
const MatrixBodyCell = (props) => {
    const { row, column, columnIndex } = props;
    const config = useMatrixConfig();
    const { renderCell, renderCellWrapper, selectable, addLabel, occupiedAddAffordance, touch, classes, } = config;
    const store = useMatrixInteraction();
    const cell = row.cells[column.key];
    const context = useMemo(() => ({ row, column, columnIndex }), [row, column, columnIndex]);
    ...
    const getSnapshot = useCallback(() => store.getCellState(row.key, column.key), [store, row.key, column.key]);
    const state = useSyncExternalStore(store.subscribe, getSnapshot);
    const handleMouseDown = useCallback((event) => {
        if (event.button !== 0)
            return; // right/middle click is not a range
        ...
        if (event.target instanceof Element &&
            event.target.closest("[draggable=true],button,a,input,select,textarea,[role=button],[contenteditable=true]"))
            return;
    ...
    const chip = cellSelectable && occupied && (occupiedAddAffordance === "chip" || touch);
    const overlay = cellSelectable && !touch && occupiedAddAffordance === "overlay"
        ? occupied
        : false;
    const chipActivation = useMatrixActivation(chip ? addToCell : undefined, typeof addLabel === "string" ? addLabel : undefined);
    const stopPress = useCallback((event) => {
    ...
    return (_jsxs(MatrixBodyCellRoot, { className: combineClassNames([
            classes?.cell,
        ...
        onMouseEnter: selectable ? handleMouseEnter : undefined, onMouseLeave: selectable ? handleMouseLeave : undefined, children: [content, chip && (_jsx(MatrixAddChip, { "data-cc-matrix-add-chip": "", className: combineClassNames([
                    classes?.addChip,
                    touch && matrixClasses.touch,
                ]), title: typeof addLabel === "string" ? addLabel : undefined, ...chipActivation, onMouseDown: stopPress, children: _jsx(Add, { fontSize: "small" }) })), !chip && cellSelectable && !!(state & MATRIX_CELL_ADD_HINT) && (_jsx(MatrixAddHint, { className: combineClassNames([
                    classes?.addHint,
                    overlay && matrixClasses.addHintHalf,
                ]), children: addLabel }))] }));
};
export default React.memo(MatrixBodyCell);

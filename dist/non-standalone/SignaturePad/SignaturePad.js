var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __rest = (this && this.__rest) || function (s, e) {
    var t = {};
    for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p) && e.indexOf(p) < 0)
        t[p] = s[p];
    if (s != null && typeof Object.getOwnPropertySymbols === "function")
        for (var i = 0, p = Object.getOwnPropertySymbols(s); i < p.length; i++) {
            if (e.indexOf(p[i]) < 0 && Object.prototype.propertyIsEnumerable.call(s, p[i]))
                t[p[i]] = s[p[i]];
        }
    return t;
};
import React, { useCallback } from "react";
import SignPad from "../../standalone/SignPad/index";
import { useDialogContext } from "../../framework";
import { SignDialog } from "../Dialog";
var SignaturePadCanvas = function (props) {
    var signature = props.signature, disabled = props.disabled, openInfo = props.openInfo, dialogProps = __rest(props, ["signature", "disabled", "openInfo"]);
    var pushDialog = useDialogContext()[0];
    var showSignDialog = useCallback(function () {
        if (disabled)
            return;
        pushDialog(React.createElement(SignDialog, __assign({ signature: signature }, dialogProps)));
    }, [pushDialog, disabled, signature, dialogProps]);
    return (React.createElement(SignPad, { openSignPad: showSignDialog, signature: signature, disabled: disabled, openInfo: openInfo }));
};
export default React.memo(SignaturePadCanvas);
